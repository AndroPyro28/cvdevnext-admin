...
export default function ProfileClient({ params }) {
    ...
    const handleProfileUpdate = async () => {
        ...
        try {
            const response = await axios.put(`${process.env.NEXT_BACKEND_URL}/api/home-owner/profile/${userId}/password`, updatedData);
            const responseData =  response.data;
          
            // Re-fetch updated data
            const fetchUserData = async () => {
                try {
                    const response = await axios.get(`${process.env.NEXT_BACKEND_URL}/api/home-owner/profile/${userId}`);
